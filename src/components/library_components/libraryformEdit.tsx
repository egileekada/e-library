...
import { useEditBookCallback, useEditJornalCallback, useEditReportCallback, useUploaderCallback } from '../../connections/useaction';
...
function LibraryformEdit(props: Props) {
    const {
        close,
        data,
    } = props
    ...
    const { handleEditBook } = useEditBookCallback();
    const { handleEditJornal } = useEditJornalCallback();
    const { handleEditReport } = useEditReportCallback();
    ...
    const addMutation = useMutation(async (formData: ILibrary) => {
        ...
        if (type === "JOURNAL") {
            response = await handleEditJornal(formData, data?.id + "");
        } else if (type === "BOOK") {
            response = await handleEditBook(formData, data?.id + "");
        } else {
            response = await handleEditReport(formData, data?.id + "");
        }
    ...
    const submit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!formik.dirty || !formik.isValid) {
            ...
            if (type === "Report") {
                addMutation.mutateAsync({ ...reportData, thumbnail: data?.thumbnail }, {
                    onSuccess: (data: any) => {
                        if (data) {
                            // close(false)
                        }
                ...
                addMutation.mutateAsync({ ...BookData, thumbnail: data?.thumbnail }, {
                    onSuccess: (data: any) => {
                        if (data) {
                            // close(false)
                        }
                ...
                addMutation.mutateAsync({ ...literatureData, thumbnail: data?.thumbnail }, {
                    onSuccess: (data: any) => {
                        if (data) {
                            // close(false)
                        }
        ...
        <form style={{ width: "full" }} onSubmit={(e) => submit(e)} >
            <Flex w={"full"} gap={"4"} flexDir={"column"} pb={"4"} >
                ...
                <Box w={"full"} >
                    ...
                    <Select value={type + ""} disabled={true} fontSize={"14px"} bgColor="#FCFCFC" borderColor="#BDBDBD" _hover={{ borderColor: "#BDBDBD" }} _focus={{ backgroundColor: "#FCFCFC" }} focusBorderColor="#BDBDBD" height={"45px"}>
